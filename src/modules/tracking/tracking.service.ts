...
import { ipClient } from "../../config/axios.js";
import prisma from "../../config/prisma.js";
import { sendEmail } from "./tracking.mailer.js";
import { BatchStatsResult, CreateBatchDto, IpApiResponse } from "./tracking.type.js";
...
class TrackingService {
  ...
  async trackOpen(trackingId: string, req: { clientIp?: string; publicIp?: string; headers: Record<string, string | string[] | undefined> }) {
    ...
    const ipInfo = await this.ipLookup(req.clientIp ?? "");
  ...
  async ipLookup(ip: string) {
    const { data } = await ipClient.get<IpApiResponse>(`/${ip}/json`);
    return data;
  }
}
